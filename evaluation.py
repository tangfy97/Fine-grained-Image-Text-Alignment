...
def evalrank(model_path, data_name, split='dev', fold5=False):
    """
    ...
    voca = vocab.deserialize_vocab(os.path.join("../artworks/artworks", 'vocab.json'))
    opt.vocab_size = len(voca)

    # construct model
    mode = model.SCAN(opt)

    # load model state
    mode.load_state_dict(checkpoint['model'])

    print('Loading dataset')
    data_loader = data.get_test_loader(split, opt.data_name, voca,
                                       opt.batch_size, opt.workers, opt)

    print('Computing results...')
    img_embs, cap_embs, cap_lens = encode_data(mode, data_loader)
...
if __name__ == '__main__':
    evalrank("/home/ljd/PycharmProjects/assignment_4_13/image-text/runs/artworks_scan/log/model_best.pth.tar", "../")